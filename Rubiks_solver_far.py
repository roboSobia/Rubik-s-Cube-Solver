...
def main():
    ...
    print("Automatically scanning the U face 12 times from different angles.")
    print("Colors: B=Blue, Y=Yellow, R=Red, G=Green, W=White, O=Orange")
    print("Press 'q' to quit, 'r' to restart, 'a' to calibrate.")
    ...
    prev_contour = None
    stability_counter = 0
    STABILITY_THRESHOLD = 10  # Number of frames the cube must be stable
    MIN_CONTOUR_AREA = 8000   # Slightly reduced to allow angled views
    ...
    while True:
        ...
        grid_size = int(min(height, width) * 0.25)  # Smaller grid for flexibility
        grid_cell_size = grid_size // 3
        ...
        for contour in contours:
            ...
            if area > MIN_CONTOUR_AREA:
                ...
                if len(approx) == 4:
                    # Relaxed aspect ratio check to allow angled views
                    x, y, w, h = cv2.boundingRect(approx)
                    aspect_ratio = w / float(h)
                    if 0.5 < aspect_ratio < 2.0 and abs(w - grid_size) < 100:  # Wider range
                        cube_detected = True
                        cv2.drawContours(display, [approx], -1, (0, 255, 0), 2)
                        
                        # Stability check
                        if prev_contour is not None and cv2.matchShapes(contour, prev_contour, 1, 0.0) < 0.15:
                            stability_counter += 1
                        else:
                            stability_counter = 0
                        prev_contour = contour
                        
                        # Perspective transform to correct the angled view
                        ...
                        # Define destination points for a square grid
                        dst = np.array([
                            [0, 0],
                            [grid_size - 1, 0],
                        ...
                        # Draw the 3x3 grid on the display (not warped image)
                        pad_x, pad_y = x, y
                        grid_cell_size = grid_size // 3
                        for i in range(1, 3):
        ...
        # Display instructions or status
        if current_scan_idx < 12:
            if cube_detected:
                status = f"Detecting U face #{current_scan_idx + 1}/12 - Stable: {stability_counter}/{STABILITY_THRESHOLD}"
            else:
                status = f"Scan U face #{current_scan_idx + 1}/12 - Position cube at an angle"
        ...
        # Automatic capture when cube is stable
        if cube_detected and stability_counter >= STABILITY_THRESHOLD and current_scan_idx < 12:
            # Use the warped (corrected) image for color detection
            image = warped.copy()
            
            # Detect colors on the corrected grid
            face_colors = []
            print(f"\nChecking U face scan #{current_scan_idx + 1} (3x3 grid):")
            for i in range(3):
                row = []
                for j in range(3):
                    y_start = i * grid_cell_size
                    y_end = (i + 1) * grid_cell_size
                    x_start = j * grid_cell_size
                    x_end = (j + 1) * grid_cell_size
                    roi = image[y_start:y_end, x_start:x_end]
                    color = detect_color(roi, color_ranges)
                    face_colors.append(color)
                    row.append(color)
                    # Overlay color on the warped image for visualization
            ...
            is_duplicate = False
            for prev_scan in u_scans[:current_scan_idx]:
                if prev_scan and prev_scan == face_colors:
                    is_duplicate = True
                    print(f"Duplicate face detected, skipping scan #{current_scan_idx + 1}")
                    break
            
            if not is_duplicate:
                print(f"Automatically captured U face scan #{current_scan_idx + 1}")
